...
DWORD WINAPI ProcessClient(LPVOID arg)
{
	...
	SOCKET client_sock = ptr->client_sock;
	char buf[BUFSIZE+1];
	//SOCKADDR_IN clientaddr;
	//int addrlen;
	int retval;
	...
	while(1){
		...
		printf("[TCP/%s:%d] %s\n", inet_ntoa(ptr->clientaddr.sin_addr),
			ntohs(ptr->clientaddr.sin_port), buf);
...
int main(int argc, char* argv[])
{
	...
	while(1){
		...
		// 사용자 정의 구조체를 동적으로 할당한다.
		ClientInfo *ptr = (ClientInfo *)malloc(sizeof(ClientInfo));
		ptr->client_sock = client_sock;
