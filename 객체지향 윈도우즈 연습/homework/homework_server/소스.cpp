...
void Binding_serveraddr(SOCKADDR_IN *serveraddr, int len, int &retval, SOCKET* listen_sock){
	ZeroMemory(serveraddr, len);										// serveraddr 구조체를 초기화한다.
	serveraddr->sin_family = AF_INET;													// 주소 패밀리를 ipv4로 설정한다.
	serveraddr->sin_port = htons(9000);													// 서버가 바인딩될 포트를 설정한다.
...
int main(int argc, char* argv[]){
	...
	Binding_serveraddr(&serveraddr, sizeof(serveraddr), retval, &listen_sock);
	// 문제
